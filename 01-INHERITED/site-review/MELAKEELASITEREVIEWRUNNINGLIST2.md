...
**Companions:** `MELA-KEELA-LANGUAGE-RESEARCH-PROMPTS.md` contains eleven bounded research prompts, one shared evidence contract, a challenge/revision prompt and a Claude Code integration prompt. `MELA-KEELA-CLAUDE-FULL-SEQUENCE.md` gives the complete research-ZIP-to-repository workflow.
...
**Yes: use separate Claude Chat research sessions for bounded packets, then hand reviewed structured content to Claude Code. Do not ask many isolated chats to generate finished HTML pages and dump them into the repository.** The latter increases duplicated claims, conflicting dates, inconsistent components and discarded local corrections.

1. **Claude Code inventory first:** Read current repository instructions, routes, redirects and relevant pages; identify exact extend/merge/new dispositions and prior corrections. Produce an inventory only. This prepares research inputs without redesigning or deploying the museum.
2. **Shared research contract:** Give every Claude Chat session the same programme brief, relevant existing-page text, current correction ledger and required output structure. Supply only the actual evidence assets available; a link not opened is not a reviewed source.
3. **Eleven topic packets:** Run R1–R11 separately. Begin with R1 and R9; use R9 before R3, and R9/R3 before final Pāṇini copy. Run R10 before corpus-wide Rigvedic social claims. Follow with R2, R4, R11, R5, R6, R8 and the directionality-intensive R7. These are separate research conversations, not page commitments.
4. **Evidence challenge and revision:** Use the challenge prompt on each packet. Reopen cited sources and check claims, locators, chronological logic, counterarguments and numeric calculations. A second model's agreement is not verification. Reserve specialist review for claims requiring linguistic, epigraphic or textual expertise.
5. **Content consolidation:** Resolve contradictions across packets once; maintain shared IDs, date ranges, terminology and rights. Produce the route manifest and accepted page copy. Preserve unresolved findings as research items rather than quietly simplifying them for the build.
6. **Claude Code integration:** Provide the consolidated packet, accepted scope and asset references. Implement the smallest complete batch using repository components; update navigation/search/ledger/redirects and check mobile, keyboard, Tamil rendering, source links and correction propagation. Return a reviewable preview and diff before deployment.
...
**Operational decision:** Expand the companion prompt pack from eight to eleven packets and use the full sequence in `MELA-KEELA-CLAUDE-FULL-SEQUENCE.md`. Claude Chat produces researched Markdown/CSV/JSON packets; Claude Code integrates reviewed packets into the current repository. Independent chat-generated HTML is not the normal handoff.
...
Run the Claude Code inventory before research copy. Run R9 before R3, and R3/R9 before finalizing R2’s public narrative. Run R10 before treating any corpus-wide Rigvedic social claim as a single-period finding. Run R11 separately because etymology and typological comparison require different sources from metrical phonology.
