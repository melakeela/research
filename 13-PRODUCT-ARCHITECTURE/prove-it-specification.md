...
3. **One worked case**, at §5: *did Indra's poets destroy ninety-nine forts?*
   Built end to end, with every claim identifier named, the status floor
   computed, the offered conclusions derived and the refused conclusions
   enumerated with their reasons.
...
| `rests_on[]` | **Non-empty.** Pinned `mk:clm:` references, each of which is in `claims[]`. |
| `derivation_note` | the inferential step from those rows to this statement, in one or two sentences, introducing no proposition not in `rests_on[]` |
| `offer_ceiling` | derived: the **lowest** status among `rests_on[]`. Displayed with the option. |
...
| `reason_rests_on[]` | the claims, absences, holds or gate results the refusal itself rests on, pinned |
...
**W-R1. Withholding on a weak reason is forbidden.** Where `why_absent =
gate-failed` and `reason_status` is below the institution's `accepted_threshold`
(D-010), the conclusion is **not withheld**. It moves into `conclusion_set[]`
with the gate result attached and rendered against it. Refusing a conclusion is
the strongest thing this interface does, and doing it on an assumption the
institution has not itself verified is the mode's characteristic failure in the
direction nobody watches. §6.2 records the instance of this that was caught in
drafting this document.

The other five `why_absent` values are not subject to W-R1: they are statements
about what the packet contains, not judgements about the past.
...
The cost of the answer taken is stated at §6.1: a listed option is one click and
the free-text escape is work, so the constrained set has a thumb on the scale in
favour of readings the institution has already written down. §6.1 is the control.
...
- **Every Challenge still emits its own `puts-in-evidence` records.** EOM §9.4 is
  not relaxed: those links carry the Challenge as subject and are what the
  Challenge grounds on. They are **generated from the packet** and each carries
  the packet id and revision. So the selection is shared and auditable, and the
  assertion of it stays attributable to the surface that made it.
...
| `date_range` | Composition of the Rigvedic hymns, undated in absolute terms in this packet; internal relative order via Arnold's five periods, which is itself `HYPOTHESIS` (`PUR-028`). Editions: Aufrecht (`SRC-020`), Lubotsky/padapāṭha (`SRC-021`), Zürich annotation (`SRC-022`), Arnold 1905 (`SRC-026`), Griffith 1890 (`SRC-073`), Geldner 1951 (`SRC-072`), Grassmann 1876–7 (`SRC-074`). |
...
| `null_explanation` | Nothing about any event needs to be posited: the six passages are fully explained by the poetic tradition's own stock of numbers and its formulaic diction. |
...
| *Indra's poets* | **No.** In the text Indra breaks the forts, not the poets. No claim here bears on any human agent, and there is no route from this packet to one. |
...
`status_census`: **`VERIFIED` 26 · `PROVISIONAL` 13 · `HYPOTHESIS` 2 ·
...
The census is the other half of the reading: 26 of 41 rows are verified
...
**A stone fort.** RV 4.30.20: `śatám aśmanmáyīnām purā́m índro vy àsyat /
...
not ninety-nine** (`PUR-N-024`). The most concrete fort in the corpus is not in
the ninety-nine set.
...
**against** the instruments, each naming on the row what it overrides; one of
them, RV 2.19.6, is a ninety-nine row.
...
simplex stanzas, 70 simplex hymns, 78 string hits, 48 padapāṭha resolutions, 156
admitting `púraṃdhi-`, 107 admitting `purāṣáh-`. The nearest are 103 and 104
(`06-BRIEFS/rv01-reconciliation.md` §2.4). A full-text scan of every tracked file
finds no corpus operation anywhere in this repository that yields 99 (§2.5–2.6).
...
predictions for an enumeration reading; two fail decisively, one holds, one is
inconclusive. It **fails** that an enumeration should not migrate to another class
...
single most instructive thing this mode can teach"*, and this case is a good
teacher:

```
14 source ids on the counting rows
└── one clone, VedaWeb @ d3eb8af  (SRC-019 = SRC-069 = SRC-085, DEP-024)
    ├── Aufrecht Saṃhitā (SRC-020)
    ├── padapāṭha (SRC-021)
    ├── Zürich morphology (SRC-022)  ── glosses from Grassmann's Wörterbuch
    ├── strata.json (SRC-023) ──── a transcription of ────┐
    ├── addressees.json (SRC-070)                         │
    └── translations                                      │
        ├── Griffith 1890 (SRC-073)     independent       │
        ├── Geldner 1951 (SRC-072)      independent       │
        ├── Grassmann 1876-7 (SRC-074) ─┐                 │
        └── Elizarenkova (SRC-075) ── made with Geldner in view (DEP-023)
                                        │                 │
   Grassmann's translation and his Wörterbuch are ONE source (DEP-021)
   Arnold 1905 (SRC-026) is the strata's actual root (DEP-001) ───────┘
   stanza_properties' 'arnold' column is Arnold 1897 — same man (DEP-022, BF-013)
```

Read plainly: **fourteen source ids, one repository, three independent
translating voices of which one is also the lexicon that drew the family
boundary, one metrical periodisation, and no non-European scholarly source
anywhere.** Sāyaṇa's commentary — the indigenous exegetical tradition, and the
one Griffith leaned on — is not in the pinned corpus and was not reachable
(`RA-015`, `HIGH`, blocked on egress). Where a count row reads "3/4 translators",
that is Griffith, Geldner and Grassmann; where it reads "1/4", it is Griffith
alone.
...
| Poet lineage for 5 of the 6 passages | `NOT PRODUCED` | Geldner's headings name a deity, not a poet, for 45 of 103 (`PUR4J-016`). An arrangement's silence is not anonymity. Blocked: **`HOLD-006`**. |
...
| Archaeological fortification, inferred geography, unsupported identification | `NOT ASSIGNED` | Three of §4J's five types are verdicts on arguments nobody has made yet (`PUR4J-024`). |
...
|---|---|---|---|---|
| **C-1** | "The Rigveda states ninety-nine forts in six passages, and one hundred in nine. Ninety-nine is not the number it gives most often." | `PUR4J-001`, `PUR4J-002`, `PUR4J-003`, `PUR4J-004` | `VERIFIED` | `agree` |
...
| **C-4** | "Both: a conventional number attached to fighting the tradition remembered. The passages are narrative even where the count is formulaic." | `PUR4J-026`, `PUR4J-I-01`, `PUR4J-025` | `PROVISIONAL` | `disagree` |
| **C-5** | "Ninety-nine is prominent today because `náva navatí-` is a striking phrase and Griffith rendered it strikingly — a fact about the expression, not only about reception." | `PUR4J-004`, `PUR4J-I-02` | `PROVISIONAL` | `disagree` |
| **C-6** | "The counting passages are a late layer in the Rigveda." | `PUR-014`, `PUR-016`, `PUR-017`, `PUR-018`, `PUR-026`, `PUR4J-011`, **`PUR-028`** | **`HYPOTHESIS`** | `disagree` |
| **C-7** | "The question is malformed: in these passages Indra breaks the forts, and nothing here bears on any human doing anything." | `PUR4J-020`, `PUR4J-025`, `PUR4J-024` | `PROVISIONAL` | `question-is-malformed` |
...
**C-6 is the one to look at.** It is offered with a `HYPOTHESIS` ceiling and with
the stratum evidence rendered *against* it, because the reason to refuse it —
that the púr- vocabulary is over-represented in the earliest stratum — is a gate
result resting on `PUR-028`, which the institution has not verified. Under
**W-R1** that does not withhold; it annotates. §6.2 records that this document's
first draft had C-6 in the withheld set, and why that was a failure.

### 5.8 Stage 7 — YOU DECIDE: what is refused, and why

Six, rendered inline at §3.10's fixed small footprint, never folded (W-R2).

| # | Statement, in the words a visitor would use | `why_absent` | `what_would_admit_it` | Reason rests on | Reason status |
|---|---|---|---|---|---|
| **W-1** | "Ninety-nine forts were destroyed." | `bridge-not-established` | A claim tying any one of the six passages to a destruction event — a stratigraphic horizon, a datable burning, a named place. No such claim exists in this packet, and the packet's `evidence_classes_required` does not include material evidence. | `PUR4J-024`, `PUR4J-018`, scope | `PROVISIONAL` |
...
| **W-4** | "The forts belonged to the Dāsas, so the Dāsas were a people who lived in forts." | `bridge-not-established` | A claim attaching a named group to a fort **by the syntax of a passage**, not by sharing a stanza with it. `dāsī́-` co-occurs in family stanzas at 43× its corpus rate, and that is a co-occurrence statistic; the 22 opponent-side names are Grassmann's classification of the *name*, not a reading of any passage's grammar. `BF-014` is the logged instance of this exact inference being made in this repository and being wrong. | `PUR-022`, `PUR4J-030`, `BF-014`, `RA-014` | `VERIFIED` |
...
late"*; the inherited RV-01 specification is titled *"The 99 Forts Database"*;
Version 12 carries the figure at five separate lines. The interface refuses the
...
**W-4 is the one that matters most.** It is the conclusion a visitor is most
likely to reach unaided, it is where both the Aryan-invasion narrative and its
mirror enter, and its refusal rests on a `VERIFIED` measurement plus a logged
instance of this institution making that exact inference and being wrong. The
Constraint Block backs it independently: `identity_attribution = none` on every
experience object (EOM §5.2), so a surface attributing a people to a fort could
not be written down even if a designer wanted it.
...
Three prerequisites. Rule S-3 forbids creating any of them here, and naming them
is the point of building the case rather than describing it.
...
3. **Translation Blocks** (§3.8) for `púr-`, `navatí-`, `śatatamá-` and
   `ā́yasa-`, each complete in all eleven fields including
   `inherited_category_audit` — and *fort* is on the constitution's own list of
   English categories to audit. `06-BRIEFS/pur-translation-standard.md` is the
   nearest thing in hand and is not the block.

The research for this case is finished to a standard most of this repository has
not reached. It is the **product-layer records** that are missing, and they were
invisible until an instance was built.
...
and `counter_to_institution` is a field, so the split framework §11.6 rule 5 asks
for can be computed. Logged as `BF-024`, corrected inside this document.
...
the best-evidenced corner of this repository — 41 claim rows, a pinned corpus, an
adjudicated count register — and picking it makes the mechanism look easy. A
packet on a subject with four `INHERITED-UNVERIFIED` rows and one blocked source
would have exercised `hold_disclosure`, `INHERITED-UNVERIFIED` floors and an
almost-empty `conclusion_set[]`, and none of those paths is tested here. Stated,
not corrected: one case was asked for and one was built. The untested paths are
named so a second packet is built against them rather than against another
well-evidenced subject.
...
A second, smaller one: the worked case gives reading (b), enumeration, two
...
Logged: `BF-024`, `BF-025` in `04-AUDITS/BIAS-FAILURE-LOG.csv`; `RA-022` in
`04-AUDITS/REAUDIT-QUEUE.csv`; `IC-P-002` in
...
|---|---|---|
| `D-010` | which institutional claims may presently be published | sets `accepted_threshold`, which W-R1 is written against. **Until it is answered, W-R1 cannot be evaluated and no packet's withheld set can be validated.** Load-bearing. |
...
12 lines — is `INHERITED-UNVERIFIED` and is marked as such where it is used.
