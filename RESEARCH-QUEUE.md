...
| J | The Rigvedic forts | Translating *pur* into a Mature Harappan city; one famous count standing for the corpus — *corpus built 2026-09-07 at the stanza unit and independently reproduced byte-for-byte from commit `d3eb8af`; 103 passages, all fifteen enumerated items addressed and the five-way typology assigned. 99 is stated in 6 passages and is not modal — 9 passages state a* `śatá-` *count. The 99 expression is corpus-wide rather than a fort formula: 19 stanzas over seven books, of which 12 count rivers, arms, horses, coils and oblations. RV 6.47.2 states 99 of Śambara's* `dehī́-` *ramparts, so the figure is 6 on the etymological reading of the corpus and 7 on the semantic one:* `RA-016`. *The named trap is gated as* `HYP-J-001` — *status* `HOLD`, *reading* `NOT-ELIGIBLE-SOURCE-BLOCKED`: *Nandi's chapter is paywalled and blocked on both channels (*`HOLD-007`, `D-046`*), two of the five legs cannot be run, and the four-term list itself is on hold (*`DJ-010B`*). **Independent adversarial review overturned two results and the source behind every lexical claim, all of it in this unit's favour** — see* `BF-017` *and* `06-BRIEFS/domain-j-forts.md` *§0a; three re-audits are open (*`RA-018`, `RA-019`, `RA-020`*). Poet lineage rests on Geldner alone (*`HOLD-006`*); proposed geography is* `NOT PRODUCED`. |
