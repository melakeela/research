...
| J | The Rigvedic forts | Translating *pur* into a Mature Harappan city; one famous count standing for the corpus — *corpus built 2026-09-07 at the stanza unit and independently reproduced byte-for-byte from commit `d3eb8af` in a later session; 103 passages, all fifteen enumerated items addressed and the five-way typology assigned. 99 is stated in 6 passages and is not the modal count; 100 is, in 8. The 99 expression is corpus-wide rather than a fort formula — 19 stanzas, of which 12 count rivers, arms, horses, coils and oblations. RV 6.47.2 states 99 of Śambara's* `dehī́-` *ramparts, so the figure is 6 on the etymological reading of the corpus and 7 on the semantic one: reconciliation C-2, queued as* `RA-016`. *The named trap is gated as* `HYP-J-001`, *verdict* `NOT-ELIGIBLE-SOURCE-BLOCKED` — *Nandi's chapter is paywalled and blocked on both channels (*`HOLD-007`, `D-046`*), and two of the five gate legs, chronology and geography, cannot be run from anything this repository holds. Poet lineage rests on Geldner alone (*`HOLD-006`*); proposed geography is* `NOT PRODUCED`. *See* `06-BRIEFS/domain-j-forts.md` *and* `06-BRIEFS/pur-4j-corpus.md`.* |
