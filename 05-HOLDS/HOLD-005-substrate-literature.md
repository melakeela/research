# HOLD-005 — the substrate literature itself is unretrieved
...
| Krishnamurti, *The Dravidian Languages*, CUP 2003 | the Proto-Dravidian reconstructions used at one remove as `SRC-062` | archive.org |
| Rau 2019 (Proto-Munda) | the reconstructions used at one remove as `SRC-064` | — |
| Shorto, *A Mon-Khmer Comparative Dictionary*, 2006 | the deeper-Austroasiatic comparanda carried inside `SRC-064` | — |

## Why it is blocked

Every plausible host is refused at the egress gateway in this session:
`archive.org` (`SRC-053`), `www.jstor.org` (`SRC-032`),
`www.ejvs.laurasianacademy.com`, `dsal.uchicago.edu` (`SRC-056`),
`gretil.sub.uni-goettingen.de` (`SRC-054`),
`titus.uni-frankfurt.de` (`SRC-055`). Both available channels — `curl`
...
(`SRC-058`), serves public git repositories and nothing else.
