...
**Page:** *Veli: The Open, and What Was Put Outside* · MVP rank **4** of 15 · `Nocturnal Veḷi` · Decision `Keep` · Risk `Low`
...
|---|---|---|---|
| `IH-062` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L176` | Owner decision HD-01, status recorded as SETTLED: the name is Tamil *veḷi*, retroflex *ḷ*, romanised *veli*; the Telugu excommunication sense is *'treated as a feature'*. Note what this is — a naming decision, not a lexical finding. It settles what the institution is called, not what the word means. |
| `IH-314` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L552` | Work item 46: Tamil-dictionary and native-speaker confirmation is **owed** for *mela/kila, suvadu, adukku, nokku, tinai, navalam* and *porul*. The semantic work behind the brand vocabulary is recorded as not done. |
...
| Field | Value |
|---|---|
| Required asset set | manuscript/inscription image; glyph diagram; language map; pronunciation audio where licensed |
...
1. A Translation Block for *veḷi* exists to the constitution §7 standard: original script, transliteration, grammatical form, semantic range, textual context, edition, exact locator, translation used, alternative translations, and the interpretive consequence of choosing between them. None of that is in this repository today.
2. *'One word, not six'* is stated as what it is — an etymological and distributional claim, with DEDR entry numbers, and with the homonymy alternative stated rather than omitted.
3. The single-digitisation dependency is recorded in `02-SOURCES/dependency.csv` and the claim is presented at `PROVISIONAL`, not `VERIFIED`, until a second independent lexicographic line is read.
4. The Telugu sense is either evidenced from a Telugu source or removed. `IH-062` treats it as *'a feature'* of the name — a branding judgement, which is not evidence that the sense exists as described.
5. Any pronunciation audio has a Consent Register row (§11.4). No licence substitutes for it.
