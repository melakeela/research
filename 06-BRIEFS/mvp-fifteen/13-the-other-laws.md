# `the-other-laws` — page brief

**Page:** *Law Beyond the Dharmasastra: Other Legal Traditions* · MVP rank **13** of 15 · `Tamil Retrofuture` · Decision `Split` · Risk `Low`
...
|---|---|---|---|
| `IH-120` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L250` | Provisional finding P-12, from Aktor 2018 and Davis 2020/2022/2024 — sources the handoff marks **STRONG**: dharmaśāstra shows intent to control mobility; caste was produced by legal rules; penalties were differential. Entered here as `INHERITED-UNVERIFIED`. |
| `IH-046` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L134` | Correction C-26/R-20: the demotion event. Kabir Babu 2016, Choudhury 2021 (MDPI preprint) and Kumar & Choudhury 2020 were left carrying claims that *Aktor, Davis, Olivelle, Beteille and the corpus actually support*. The re-attribution on `birth-was-not-always-destiny` and `jatization` was owed next pass and is listed among the **unconfirmed** items — and those are this page's neighbours. |
...
1. **The split is executed.** The page cannot launch in its audited form, because its audited decision is that it should not exist in that form. What the split's shape is has not been decided by anyone.
2. The Editorial Register (§11.5) exists to hold that decision. `SCHEMA.md` §7: *nothing in `03-REGISTERS/` records a publication decision.* A `Split` with no register to record it is an instruction with no addressee.
3. The four claims in the H1 are separated, each with its own chronology, evidence class and status (method steps 2 and 10).
4. *'Dozens'* is a count with an inventory and an inclusion rule behind it, or it goes.
5. Aktor, Davis and Olivelle are read directly and cited at claim level, closing the re-attribution that `IH-046` records as owed and unconfirmed.
6. Both bias tests are run and logged with the asymmetry statement (§3.11, §11.2).
7. Right-of-reply positions are set for every named modern institution or scholar (§11.3), governed by **D-025** and `OWNER-DECISIONS.csv` **D-010**.
