# `sound-changes` — page brief

**Page:** *Sound Changes: Dravidian Phonemes Sanskrit Could Not Write* · MVP rank **11** of 15 · `Living Signal Field` · Decision `Revise` · Risk `Medium`
...
|---|---|---|---|
| `IH-103` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L228` | The handoff's finding V-20: Sanskrit lacks the phonemes ḷ (retroflex), ṟ (alveolar trill) and ṉ (alveolar), **which Tamil-Brahmi invented letters for**. Recorded there as a linguistic fact; entered here as `INHERITED-UNVERIFIED`. |
| `IH-062` | `INHERITED-UNVERIFIED` | `01-INHERITED/claude-project-handoff.md L176` | Owner decision HD-01: the name *veḷi* carries the retroflex *ḷ*. The page's subject and the institution's name share a phoneme, which is why this page is linked from 55 others. |
...
1. The phonological claim and the orthographic claim are stated separately, each with its own evidence (method step 10).
2. The three tables are backed by a dataset with per-row sources — currently 0 source entries support 3 comparative tables.
3. The Tamil-Brahmi letter-invention claim is dated. *When* letters were invented for these phonemes is an epigraphic and palaeographic question with a chronology, and chronology precedes interpretation (method step 2).
4. The schematic/measured distinction is explicit on the surface, as the workbook's own required action asks: which rows are attested correspondences and which are reconstructions (the attestation gradient, constitution §4E).
5. No map colours a region or a culture by a language (§8.7).
6. Register rows exist with `supports_page = sound-changes`. Today there are none, and the adjacent verified work does not count as some.
