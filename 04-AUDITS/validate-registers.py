    ...
    for n, row in enumerate(rows, start=2):
        ...
        if "locator" in fields:
            ...
            if loc and status == "VERIFIED" and INHERITED_LOCATOR.search(loc):
                # A WARNING, not a failure, and deliberately so. Two different
                # things look identical here. A claim about the world resting
                # on an IH- row is a promotion the inheritance rule forbids -
                # that was DMB-021, and it was demoted after reading it. A
                # claim ABOUT THE RECORD - "this repository records X as owed",
                # "IH-119 is INHERITED-UNVERIFIED and is not used" - is
                # correctly located in the record, and VAR-008 is one. No rule
                # separates them, so this surfaces the class on every run and
                # RA-030 queues the reading. Failing them all would rewrite
                # sound rows to satisfy a blunt check, which is the move this
                # repository exists to refuse.
                warn(f"{rel}:{n}: VERIFIED row whose locator cites inherited "
                     f"material ({loc[:60]}). Correct if the claim is about the "
                     f"record; a promotion if the inherited row is the evidence. "
                     f"RA-030.")
